...
@hook.command("kittehhunt", autohelp=False)
def start_hunt(bot, chan, message, conn):
    ...
    message("Kitty mode activated! Befriend kitties with .pet, or shoo them away with .shoo", chan)

def set_ducktime(chan, conn):
    global game_status
    game_status[conn.name][chan]['next_duck_time'] = random.randint(int(time()) + 480*2, int(time()) + 3600*2)
...
@hook.command("nomorekitties", autohelp=False)
...
@hook.command("shoo", autohelp=False)
def bang(nick, chan, message, db, conn, notice):
    ...
    score = ""
    out = ""
    miss = ["The cat just glares at you, it deeply hates you for shooing it.", "The cat doesn't seem to notice you.", "Better luck next time.", "The cat almost considers moving, but doesn't." ]
    if not game_status[network][chan]['game_on']:
        ...
        if game_status[network][chan]['no_duck_kick'] == 1:
            out = "KICK {} {} There is no cat, what are you shooing?".format(chan, nick)
            conn.send(out)
            return
        return "There is no cat, what are you shooing?"
    else: 
        ...
        duck = "cat" if score == 1 else "cats"
        message("{} you scared a kitty in {} seconds! You have scared {} {} in {}.".format(nick, timer, score, duck, chan))
        set_ducktime(chan, conn)
...
def befriend(nick, chan, message, db, conn, notice):
    ...
    out = ""
    score = ""
    miss = ["The kitty didn't want to be friends, maybe next time.", "The kitty is offended by your attempt to pet it, maybe you can try again later", "The cat doesn't want to be pet, maybe a catnip bribe might help?", "Who knew kitties could be so picky?", "The kitty was scared, maybe give it some space and try again?", "HISSSSSS"]
    if not game_status[network][chan]['game_on']:
        return "There is no hunt right now. Use .starthunt to start a game."
    elif game_status[network][chan]['duck_status'] != 1:
        if game_status[network][chan]['no_duck_kick'] == 1:
            out = "KICK {} {} You tried petting a kitty that isn't there... weirdo.".format(chan, nick)
            conn.send(out)
            return
        return "You tried petting a kitty that isn't there... weirdo."
    else:
        game_status[network][chan]['shoot_time'] = time()
        deploy = game_status[network][chan]['duck_time']
        shoot = game_status[network][chan]['shoot_time']
        ...
        duck = "cat" if score == 1 else "cats"
        timer = "{:.3f}".format(shoot - deploy)
        message("{} you befriended a kitty in {} seconds! You have made friends with {} {} in {}.".format(nick, timer, score, duck, chan))
        set_ducktime(chan,conn)
...
@hook.command("kittehs", autohelp=False)
def ducks_user(text, nick, chan, conn, db, message):
    ...
    if scores:
        for row in scores:
            ...
            ducks["chans"] += 1
        if ducks["chans"] == 1:
            message("{} has shooed {} and befriended {} kitties in {}.".format(name, ducks["chankilled"], ducks["chanfriends"], chan))
            return
        kill_average = int(ducks["killed"] / ducks["chans"])
        friend_average = int(ducks["friend"] / ducks["chans"])
        message("\x02{}'s\x02 kitty stats: \x02{}\x02 shooed and \x02{}\x02 befriended in {}. Across {} channels: \x02{}\x02 shooed and \x02{}\x02 befriended. Averaging \x02{}\x02 shooed and \x02{}\x02 friends per channel.".format(name, ducks["chankilled"], ducks["chanfriends"], chan, ducks["chans"], ducks["killed"], ducks["friend"], kill_average, friend_average))
    else:
        return "It appears {} has not participated in the cat hunt.".format(name)

@hook.command("kittehstats", autohelp=False)
def duck_stats(chan, conn, db, message):
    """Prints cat statistics for the entire channel and totals for the network."""
    ducks = defaultdict(int)
    scores = db.execute(select([table.c.name, table.c.chan, table.c.shot, table.c.befriend])
    ...
    if scores:
        ...
        message("\x02Kitty Stats:\x02 {} shooed and {} befriended in \x02{}\x02. Across {} channels \x02{}\x02 cats have been shooed and \x02{}\x02 befriended. \x02Top Channels:\x02 \x02{}\x02 with {} shooed and \x02{}\x02 with {} petted".format(ducks["chankilled"], ducks["chanfriends"], chan, ducks["chans"], ducks["killed"], ducks["friend"], killerchan, killscore, friendchan, friendscore))
    else:
        return "It looks like there has been no kitty activity on this channel or network."
